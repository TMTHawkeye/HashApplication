...
public class TagContainerLayout extends ViewGroup {
    ...
    private int mTagiconBackgroundColor = Color.parseColor("#FF000000");

    /**
     * Selected TagView background color(default #33FF7669)
     */
    private int mSelectedTagBackgroundColor = R.color.silver;
    ...
    private boolean mEnablestar = false;
    ...
    private float mstarAreaWidth = 0.0f;
    private float mIconAreaWidth = 0.0f;

    /**
     * The padding of the cross area(default 10dp)
     */
    private float mCrossAreaPadding = 10.0f;
    private float mstarAreaPadding = 0.0f;
    private float mIconAreaPadding = 10.0f;
    ...
    private float mstarLineWidth = 0.5f;
    private float mIconLineWidth = 1.0f;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mEnablestar = attributes.getBoolean(R.styleable.AndroidTagView_tag_enable_star, mEnablestar);
        mEnableIcon = attributes.getBoolean(R.styleable.AndroidTagView_tag_enable_cross, mEnableIcon);

        mCrossAreaWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_cross_width,
                dp2px(context, mCrossAreaWidth));
        mstarAreaWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_star_width,
                dp2px(context, mstarAreaWidth));
        mCrossAreaWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_cross_width,
                dp2px(context, mIconAreaWidth));
        ...
        mstarAreaPadding = attributes.getDimension(R.styleable.AndroidTagView_tag_star_area_padding,
                dp2px(context, mstarAreaPadding));


        mCrossColor = attributes.getColor(R.styleable.AndroidTagView_tag_cross_color, mCrossColor);
        mstarColor = attributes.getColor(R.styleable.AndroidTagView_tag_star_color, mstarColor);

        mCrossLineWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_cross_line_width,
                dp2px(context, mCrossLineWidth));
        mstarLineWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_star_line_width,
                dp2px(context, mstarLineWidth));
...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private void onSetTag() {
        ...
        for (int i = 0; i < mTags.size(); i++) {
            onAddTag(mTags.get(i).mname, mChildViews.size(), R.drawable.fav_filled_svg,mTags.get(i).mbol);
        }
        postInvalidate();
    }

    private void onAddTag(String text, int position, int id,boolean bole) {
        if (position < 0 || position > mChildViews.size()) {
            throw new RuntimeException("Illegal position!");
        }
        ;
        if (mDefaultImageDrawableID != -1) {
            tagView = new TagView(getContext(), text, id,bole);
        } else {
            tagView = new TagView(getContext(), text,bole);
        }
...
//        tagView.setChangeColor(R.drawable.favourites_svg);

        tagView.setCrossAreaWidth(mCrossAreaWidth);
        tagView.setstarAreaWidth(mCrossAreaWidth);

        tagView.setCrossAreaPadding(mCrossAreaPadding);
        tagView.setstarAreaPadding(mCrossAreaPadding);

        tagView.setCrossColor(mCrossColor);
        tagView.setstarColor(mstarColor);

        tagView.setCrossLineWidth(mCrossLineWidth);
        tagView.setstarLineWidth(mCrossLineWidth);
     ...
     */
    public void setTags(List<Tagmodel> tags, List<int[]> colorArrayList) {
        mTags = tags;
...
//    }
     ...
     * @param text
     */
    public void addTag(String text,boolean  bole) {
        addTag(text, mChildViews.size(),bole);
    }
     ...
     * @param position
     */
    public void addTag(String text, int position,boolean bole) {
        onAddTag(text, position, R.drawable.fav_filled_svg,bole);
        postInvalidate();
    }
    ...
    public void chdrawable(int position, String str, boolean bool) {

        TagView tagView = new TagView(getContext(), str, bool);
        initTagView(tagView, position);
        mChildViews.remove(position);
...
//        for (int i = position; i < mChildViews.size(); i++) {
    ...
    public Boolean toggleSelectTagView(int position, boolean state) {
    ...
    public List<Tagmodel> getTags() {
        List<Tagmodel> tmpList = new ArrayList<>();
        for (View view : mChildViews) {
            if (view instanceof TagView) {
                tmpList.add(new Tagmodel(((TagView) view).getText(),((TagView) view).getBoleen()));
            }
        }
        return tmpList;
    }
    ...
    public int getTagiconBackgroundColor() {
        return mTagBackgroundColor;
    }
    ...
    public void setTagiconBackgroundColor(int color) {
        this.mTagiconBackgroundColor = color;
    }
